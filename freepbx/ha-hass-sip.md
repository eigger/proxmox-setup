...
[eigger/hass-sip](https://github.com/eigger/hass-sip) 커스텀 통합 구성요소를 사용하여 Home Assistant(HA)를 FreePBX에 **SIP 클라이언트**로 직접 등록합니다. HA 내부에서 네이티브 `media_player` 엔티티로 작동하여 제어 및 TTS 재생이 매우 간편하며, 보이스 어시스트 연동, 수신인 연락처 매핑, 자동 응답(인터콤), 복잡한 IVR 자동 응답 메뉴를 지원합니다.
...
3. 아래 설정값을 입력합니다:
   ...
   - **RTP Port (선택):** `7078`
4. 등록을 마치면 HA에 `media_player.sip_client` (또는 지정한 이름의 엔티티)가 생성됩니다.
...
- `engine` (또는 `tts_engine`): TTS 엔진 엔티티 ID (예: `tts.piper`)
...
1. 수신 호의 SIP 헤더에 `Call-Info: ...; answer-after=0` 또는 `Alert-Info: Ring Answer` 등이 포함되어 올 때.
2. `sip_contacts.json` 주소록 파일에서 발신자의 설정이 `"auto_answer": true`로 지정되어 있을 때.
...
trigger:
  - platform: state
    entity_id: binary_sensor.sip_client_active # 통화중 센서 (활성화 시 on)
    to: "on"
action:
  - service: sip.answer
    target:
...
### 스마트 음성 사서함 (Voicemail System)
15초 동안 전화를 받지 않으면 음성 사서함이 연결되어 안내 멘트와 삐 소리를 들려준 뒤, 음성을 wav 파일로 녹음하고 스마트폰 노티피케이션으로 즉시 오디오 클립을 전송합니다.
```yaml
alias: "SIP: 음성 사서함 자동 녹음 및 모바일 전송"
trigger:
  - platform: state
        ...
        sequence:
          ...
          - service: media_player.play_media
            ...
            data:
              ...
              media_content_id: "media-source://tts/tts.google_translate?message=전화를+받을+수+없습니다.+삐+소리+이후+메시지를+남겨주세요."
          # 재생 끝날 때까지 대기
          - wait_for_trigger:
              - platform: event
                event_type: sip_playback_done
          ...
          - service: sip.hangup
            target:
              entity_id: media_player.sip_client
          # 휴대폰 푸시 전송 (Companion 앱 연동)
          - service: notify.notify
            data:
              title: "새 음성 메시지 도착"
              message: "{{ state_attr('sensor.sip_client_last_call', 'last_caller') }} 님으로부터 새 음성 메시지가 도착했습니다."
              data:
