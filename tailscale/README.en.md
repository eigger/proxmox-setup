...
- The Proxmox web UI **Console** tab logs in as root automatically, no password (`agetty --autologin root` override on `console-getty.service`). Anyone with access to the Proxmox host can reach root inside this container, so it relies on Proxmox's own access control.
